# python -m unittest discover ./unittest "*_test.py"

import re
import unittest
from MsDosCommandParser import MsDosCommandParser

def format_lark_pretty_print(pretty_print):
    return '\n'.join(line for line in pretty_print.split("\n") if not re.match(r'^\s*$', line))

class MsDosCmdGrammerTest(unittest.TestCase):

    def setUp(self):
        self.maxDiff = None
        transpiler = MsDosCommandParser()
        self.parser = transpiler.get_parser()

    # ------------------------------------------------------------------------
    def test_rem_normal(self):
        inputfile_text = """
REM ECHO ---------- TEST TEXT ---------- >> %OUTPUT_LOG% 2>&1
"""
        expected_parse_tree = """
program
  command_rem
    REM
    ECHO ---------- TEST TEXT ---------- >> %OUTPUT_LOG% 2>&1
""".strip("\n")
        pretty_print = self.parser.parse(inputfile_text).pretty()
        pretty_print = format_lark_pretty_print(pretty_print)
        self.assertEqual(pretty_print, expected_parse_tree)

    # ------------------------------------------------------------------------
    def test_rem_empty(self):
        inputfile_text = """
REM
"""
        expected_parse_tree = """
program
  command_rem	REM
""".strip("\n")
        pretty_print = self.parser.parse(inputfile_text).pretty()
        pretty_print = format_lark_pretty_print(pretty_print)
        self.assertEqual(pretty_print, expected_parse_tree)

    # ------------------------------------------------------------------------
    def test_rem_whitespace(self):
        inputfile_text = """
REM 
"""
        expected_parse_tree = """
program
  command_rem
    REM
""".strip("\n")
        pretty_print = self.parser.parse(inputfile_text).pretty()
        pretty_print = format_lark_pretty_print(pretty_print)
        self.assertEqual(pretty_print, expected_parse_tree)

    # ------------------------------------------------------------------------
    def test_rem_paren(self):
        inputfile_text = """
@REM FOR /F "delims= " %%i IN ("1 2 3") DO (
@REM     FOR /F "delims= " %%i IN ("1 2 3") DO (
@REM         echo B
@REM     )
@REM )
"""
        expected_parse_tree = """
program
  command_rem
    REM
    FOR /F "delims= " %%i IN ("1 2 3") DO (
  command_rem
    REM
        FOR /F "delims= " %%i IN ("1 2 3") DO (
  command_rem
    REM
            echo B
  command_rem
    REM
        )
  command_rem
    REM
    )
""".strip("\n")
        pretty_print = self.parser.parse(inputfile_text).pretty()
        pretty_print = format_lark_pretty_print(pretty_print)
        self.assertEqual(pretty_print, expected_parse_tree)

    # ------------------------------------------------------------------------
    def test_rem_colon(self):
        inputfile_text = """
REM:MY_LABEL
"""
        expected_parse_tree = """
program
  command_rem
    REM
    :
    MY_LABEL
""".strip("\n")
        pretty_print = self.parser.parse(inputfile_text).pretty()
        pretty_print = format_lark_pretty_print(pretty_print)
        self.assertEqual(pretty_print, expected_parse_tree)

    # ------------------------------------------------------------------------
    def test_rem_paren_close(self):
        inputfile_text = """
REM)test
"""
        expected_parse_tree = """
program
  command_rem
    REM
    )
    test
""".strip("\n")
        pretty_print = self.parser.parse(inputfile_text).pretty()
        pretty_print = format_lark_pretty_print(pretty_print)
        self.assertEqual(pretty_print, expected_parse_tree)

    # ------------------------------------------------------------------------
    def test_rem_paren_open(self):
        inputfile_text = """
REM(test
"""
        expected_parse_tree = """
program
  command_rem
    REM
    (
    test
""".strip("\n")
        pretty_print = self.parser.parse(inputfile_text).pretty()
        pretty_print = format_lark_pretty_print(pretty_print)
        self.assertEqual(pretty_print, expected_parse_tree)

    # ------------------------------------------------------------------------
    def test_rem_paren_hypens(self):
        inputfile_text = """
REM------------------------------------------
"""
        expected_parse_tree = """
program
  command_rem
    REM
    -
    -----------------------------------------
""".strip("\n")
        pretty_print = self.parser.parse(inputfile_text).pretty()
        pretty_print = format_lark_pretty_print(pretty_print)
        self.assertEqual(pretty_print, expected_parse_tree)

    # ------------------------------------------------------------------------
    def test_rem_paren_hypens(self):
        inputfile_text = """
REM=====================================
"""
        expected_parse_tree = """
program
  command_rem
    REM
    =
    ====================================
""".strip("\n")
        pretty_print = self.parser.parse(inputfile_text).pretty()
        pretty_print = format_lark_pretty_print(pretty_print)
        self.assertEqual(pretty_print, expected_parse_tree)
